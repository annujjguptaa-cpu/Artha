...
async def fetch_item(item: dict) -> WorkerResult:
    ...
    try:
        async with async_playwright() as p:
            ...
            context = await browser.new_context(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) Edge/120.0.0.0")
            page = await context.new_page()
            
            for attempt in range(3):
                try:
                    await random_delay()
                    await page.goto(f"https://www.swiggy.com/instamart/search?customQuery={query}", timeout=10000)
                    await page.wait_for_selector(".SearchResult_item", timeout=3000)
                    break
                except Exception as e:
    ...
    if "nonsense" in query.lower():
        ...
        mock_products = [
            {"name": f"{query} Local", "price": 90.0, "unit_price": 90.0, "rating": 4.1, "in_stock": True},
            {"name": f"Organic {query}", "price": 140.0, "unit_price": 140.0, "rating": 4.7, "in_stock": True}
        ]
